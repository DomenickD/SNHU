...
class ScoreLogger:
    ...
    def add_score(self, score, run):
        ...
        if (
            mean_score >= AVERAGE_SCORE_TO_SOLVE
            and len(self.scores) >= CONSECUTIVE_RUNS_TO_SOLVE
        ):
            solve_score = run - CONSECUTIVE_RUNS_TO_SOLVE
            print(
                "Solved in " + str(solve_score) + " runs, " + str(run) + " total runs."
            )
    ...
    def _save_png(
        ...
        plt.plot(
            x[-average_range:],
            [np.mean(y[-average_range:])] * len(y[-average_range:]),
            linestyle="--",
